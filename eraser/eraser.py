...
# Modified Eraser class for text-based tasks
class TextEraser(object):
    # def __init__(self, args: Arguments, model: nn.Module):
    #     super(TextEraser, self).__init__()
    #     self.args = args
    #     self.model = model.eval()
    #     self.tokenizer = transformers.AutoTokenizer.from_pretrained(args.model_path)
    #     self.device = torch.device(args.device)

    #     # Initialize criterion (loss function) for language modeling task
    ...
    #     dataset = load_dataset("burkelibbey/colors")
    ...
    #     # Tokenize text input
    #     return self.tokenizer(example["text"], truncation=True, padding=True)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for batch in train_dataset:
                ...
                # Compute loss
                loss = self.loss(outputs.logits, inputs["input_ids"])
                # Backpropagation
                loss.backward()
...
# # Example usage
# args = Arguments()
# args.model_path = "Priyansh-Rishav/lmeraser/tinyllama-colorist-v1"
...
# # Initialize TextEraser instance
# eraser = TextEraser(args, model)
...
class Eraser(object):
    ...
    def __init__(self, args: Arguments, model, tokenizer):
        """
    ...
    def load_dataset(self): # to get the dataset
        # Load text dataset from Hugging Face
        dataset = load_dataset("burkelibbey/colors")
        # Preprocess and tokenize the dataset
        tokenized_dataset = dataset.map(self.tokenize_function)
    ...
    def load_prompter(self, prompter_path=None):
        """Load the pre-trained llm from files
        """
